...
function getModulesFromPackage(options) {
  fs.readFile(path.join(root, options.file))
  .catch(function () {
...
function getModulesFromDir() {
  return fs.readdir(path.join(root, 'node_modules'))
    .catch(function () {
      return [];
    });
}
...
function checkModulesInstalled(root, options) {
  return Promise
    .all([
      getModulesFromPackage(options),
      getModulesFromDir()
    ])
    .then(function (res) {
      var modulesFromPackage = res[0];
      var modulesFromDir = res[1];
      console.log(`LADEBUG:`, modulesFromPackage);
      var modulesNotFound = modulesFromPackage.filter(function (item) {
        return modulesFromDir.indexOf(item) === -1;
      });

      return modulesNotFound.length !== 0;
    });
}
...
function getDependenciesFromNodeModules(root, options) {
  return Promise.all([
    ...
    .then(function (promises) {
      var nodeModulesExist = promises[0];
      var nodeModulesEmpty = promises[1];
      ...
      if (!nodeModulesEmpty) {
        // throw a customer error
        throw new Error('Empty node_modules folder: we can\'t test ' +
          'without dependencies.\nPlease run `npm install` first.');
      }
